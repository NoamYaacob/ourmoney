...
describe('isDuplicateCandidate', () => {
  it('flags an exact match on account+date+amount+description', () => {
    const candidate: ValidImportRow = { txnDate: '2026-01-15', description: 'קניות בסופר', merchantName: null, amountAgorot: -10000 }
  ...
  it('does not flag when the amount differs by even one agora', () => {
    const candidate: ValidImportRow = { txnDate: '2026-01-15', description: 'קניות בסופר', merchantName: null, amountAgorot: -10001 }
    expect(isDuplicateCandidate(candidate, 'acc-1', EXISTING)).toBe(false)
  })

  it('does not flag when the date differs', () => {
    const candidate: ValidImportRow = { txnDate: '2026-01-16', description: 'קניות בסופר', merchantName: null, amountAgorot: -10000 }
    expect(isDuplicateCandidate(candidate, 'acc-1', EXISTING)).toBe(false)
  })
  ...
  it('does not flag a near-miss description (no fuzzy matching in M7)', () => {
    const candidate: ValidImportRow = { txnDate: '2026-01-15', description: 'קניות בסופר שופרסל', merchantName: null, amountAgorot: -10000 }
    expect(isDuplicateCandidate(candidate, 'acc-1', EXISTING)).toBe(false)
  })

  it('flags every row when re-importing the identical file', () => {
    const candidates: ValidImportRow[] = [
      { txnDate: '2026-01-15', description: 'קניות בסופר', merchantName: null, amountAgorot: -10000 },
    ]
