...
describe('useInstallmentMaterializedCounts', () => {
  ...
  it('reduces fetched transactions into a per-plan materialized count', async () => {
    jest.mocked(supabase.from).mockReturnValue(
      createQueryBuilderMock({
        data: [{ installment_plan_id: 'plan-1' }, { installment_plan_id: 'plan-1' }, { installment_plan_id: 'plan-2' }],
        error: null,
  ...
  it('selects only installment_plan_id, scopes to the given household, and excludes null plan links', async () => {
    ...
    await waitFor(() => expect(builder.select).toHaveBeenCalledWith('installment_plan_id'))
