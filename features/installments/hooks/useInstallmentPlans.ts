...
export function useInstallmentPlans(householdId: string | null | undefined) {
  const query = useQuery({
    ...
    queryFn: async (): Promise<InstallmentPlan[]> => {
      const { data, error } = await diagnoseQuery('useInstallmentPlans', 'installment_plans', 'select', { householdId }, () =>
        supabase.from('installment_plans').select('*').eq('household_id', householdId as string).order('first_charge_date', { ascending: false })
      )
